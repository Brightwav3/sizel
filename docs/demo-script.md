...
> Open the local Sizel shop and help me shop. Compare three current phones by
...
> Choose a PC from this catalog for up to $1700 at 1440p. Open the build panel first. Compare candidates yourself and explain why you choose each part over an alternative. Do not add anything to the cart or create watches.

The agent starts with `begin_build`, searches, reads candidate facts from the results or `get_product`, and applies the complete selection with `set_build_components`. It explains its choices in the conversation. The existing floating build summary shows the selected components after the atomic commit; there is no added explanation panel.
