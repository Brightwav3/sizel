...
Generated with `npx tsx scripts/generate-webmcp-docs.mjs` from `src/app/webmcp/tools.ts`. This document covers only the **15** tools exposed by the stable judge-facing demo registry. The source is authoritative if this file and runtime ever disagree.
...
| `filters` | object | Facet id to values for the selected category. |
...
| Parameter | Type | Meaning |
| --- | --- | --- |
| `view` | string | Default: category listing. Values: category, product, cart. |
...
| `maxPrice` | number | Highest price. |
| `filters` | object | Facet id to values for a single requested slot. |
...
| `game` | string | Optional game label or canonical id; supported labels include CS2, Counter-Strike 2, Fortnite and Cyberpunk. Do not combine with scenario. |

### `begin_build`

Start a PC build in place: open the build panel with a shopper brief, resolution and hard budget, without changing the current page.

**Descriptor hints:** `readOnlyHint: false`.

**Result and behavior:** Returns the opened build-panel state, exact budget/resolution and full resolution-aware or shopper-supplied slot allocation. It never selects a starting slot or part; the agent owns the build order.
...
**Result and behavior:** The current build is always the unchanged baseline; do not include it as an alternative. Returns one to three agent-supplied counterfactuals with price, budget, eligibility, compatibility blockers, availability, simulated results and deltas. Common game labels are normalized to canonical fixture ids; an unknown game returns `{ game, benchmark: "no benchmark", status: "unavailable" }` instead of invented FPS. It never recommends, creates a watch or mutates the current build.
...
| `game` | string | One game label or canonical id; unsupported games return benchmark: no benchmark. Use games for a batch. |
