  ...
  return (
    <aside className="eshop-rail" onMouseLeave={() => setOpenDept(null)}>
      ...
      {openDept && <section className="eshop-flyout" aria-label={`${activeDept?.name ?? "Catalog"} menu`}>
        <header><div><span className="eyebrow">Browse</span><h2>{activeDept?.name ?? "Catalog"}</h2></div><button onClick={() => setOpenDept(null)} aria-label="Close menu" data-tip="Close menu" data-tip-align="end"><span className="ms">close</span></button></header>
