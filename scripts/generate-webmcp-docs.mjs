...
const returns = {
  ...
  estimate_performance: 'For a complete build, returns price, power, shipping, compatibility, `fps: null`, `noise: null`, and a versioned explicitly simulated result containing average FPS/1% lows where fixtures are available. An unknown game returns `{ game, benchmark: "no benchmark", status: "unavailable" }` instead of invented FPS. It does not claim measured or real-game performance.',
  ...
  compare_build_options: 'The current build is always the unchanged baseline; do not include it as an alternative. Returns one to three agent-supplied counterfactuals with price, budget, eligibility, compatibility blockers, availability, simulated results and deltas. An unknown game returns `{ game, benchmark: "no benchmark", status: "unavailable" }` instead of invented FPS. It never recommends, creates a watch or mutates the current build.',
